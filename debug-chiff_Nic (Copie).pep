...
for_3:   LDX     loc2_3,s    ;while (compteur clé < taile clé) { 
         ...
         LDA      0,s        ;Récupérer le dessus de la pile
         ADDSP    rets_2,i   ;Déplacer SP de la valeur de retour
         STA      loc1_3,s   ;placer la valeur généré dans la var. locale
         
         
         ;Voir ce qui est retourner //////
         LDA     loc1_3,s
         STA     -20,s
         DECO    -20,s
         CHARO   ' ',i
         ...
         ;LDA     loc1_3,s         ;Récupère la valeur générée
         ;ANDA    0x00ff,i         ;masque l'octet fort
         ;STBYTEA prm1_3,sxf
         ;STBYTEA prm1_3,sxf       ;clé[x] = valeur générée
...
deb_XOR: LDX     loc2_5,s 
         ...
         ;----Appel Xor16 (4)------ 
         ;STA     arg1_4,s
         LDBYTEA loc3_5,sx   ;---<MODE SX>---- vu qu'on est local ? 
         ANDA    0x00ff,i
         STA     arg2_4,s    
         ;STA     -20,s      ;permet de voir clé[x]-------------------------------------
         ;DECO    -20,s      
         ;CHARO   ' ',i
